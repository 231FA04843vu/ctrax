...
import { sendAdminPasswordResetEmail } from '../../utils/auth'
...
export default function AdminResetPassword(){
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true)
    try {
  await sendAdminPasswordResetEmail(form.email)
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded shadow">
      ...
      <div className="mt-4 text-sm text-gray-700 flex items-center justify-between">
        <Link to="/login/admin" className="hover:underline">Back to login</Link>
      </div>
    </div>
  )
}
