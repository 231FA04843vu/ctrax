...
import { sendDriverPasswordResetEmail } from '../../utils/auth'
...
export default function DriverResetPassword(){
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true)
    try {
  await sendDriverPasswordResetEmail(form.email)
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded shadow">
      <h2 className="text-2xl font-semibold mb-4">Reset Driver Password</h2>
      ...
      <div className="mt-4 text-sm text-gray-700 flex items-center justify-between">
        <Link to="/login/driver" className="hover:underline">Back to login</Link>
      </div>
    </div>
  )
}
